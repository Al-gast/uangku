-- UangKu Phase 30: Chat Input merchant/detail save support.
...
begin
  ...
  if char_length(trim(coalesce(p_merchant, ''))) > 120 then
    raise exception 'Merchant is too long';
  end if;

  if char_length(trim(coalesce(p_notes, ''))) > 1000 then
...
revoke all on function public.create_chat_transaction_with_fee(
  text, numeric, uuid, uuid, timestamptz, uuid, uuid, numeric, uuid, text, text
) from public;
